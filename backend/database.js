...
const dbPath = path.resolve(__dirname, 'clubhub.db');
    ...
    db.get("SELECT COUNT(*) AS count FROM clubs", (err, row) => {
        ...
        if (row.count === 0) {
            ...
            rawClubs.forEach(c => {
                ...
                stmt.run(c[0], c[1], c[2], c[3], c[4], c[5], c[6], mapUrl);
            });
            stmt.finalize();
            console.log(`Database seeded with ${rawClubs.length} clubs and Map links!`);
        } else {
            console.log(`Database has ${row.count} clubs. Delete clubhub.db to reseed.`);
        }
    });
}
